...
SELECT
    g.rok AS rok,
    -- rok
    g.procent_rust_hdp AS procent_rust_hdp,
    -- % růst HDP
    w.procent_rust_mzdy AS procent_rust_mzdy,
    -- % růst mzdy
    p.procent_rust_cen AS procent_rust_cen
    -- % růst cen
FROM
    gdp_growth g
LEFT JOIN wage_growth w
        USING(rok)
    -- přidáme růst mezd
LEFT JOIN price_growth p
        USING(rok)
    -- přidáme růst cen
WHERE
    w.procent_rust_mzdy IS NOT NULL
    -- jen roky s vypočteným růstem mezd
    AND p.procent_rust_cen IS NOT NULL
    -- jen roky s vypočteným růstem cen
ORDER BY
    g.procent_rust_hdp DESC;
-- seřadíme od největšího růstu HDP
