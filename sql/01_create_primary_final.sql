...
WITH
  price_yearly AS (
    SELECT
      ...
      EXTRACT(YEAR FROM date_from) AS rok,       -- rok z data (date_from)
      AVG(value)             AS prumerna_cena    -- průměrná cena v daném roce
  ...
  payroll_yearly AS (
    SELECT
      payroll_year           AS rok,             -- rok výplaty
      industry_branch_code   AS industry_code,   -- kód odvětví
      AVG(value)             AS prumerna_mzda    -- průměrná mzda v daném roce a odvětví
    FROM data_academy_content.czechia_payroll
    WHERE value_type_code = 5958                  -- filtrujeme jen platy (kód 5958)
    GROUP BY
      payroll_year,
...
SELECT
  pr.rok                                AS rok,              -- rok
  ib.name                               AS odvetvi,          -- název odvětví
  ROUND(pr.prumerna_mzda::numeric, 2)   AS plat,             -- průměrná mzda zaokrouhlená na 2 deset. místa
  pc.name                               AS potravina,        -- název potravinové kategorie
  ROUND(py.prumerna_cena::numeric, 2)   AS prumerna_cena     -- průměrná cena zaokrouhlená na 2 deset. místa
FROM payroll_yearly    AS pr
  -- připojíme názvy odvětví
  JOIN data_academy_content.czechia_payroll_industry_branch AS ib
    ON pr.industry_code = ib.code
